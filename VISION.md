...
### Files, not folders
The sidebar is a live navigable tree — with breadcrumbs up the tree, a fuzzy-jump command palette (Ctrl+P), sortable columns (name / size / modified / kind) with persistent ordering, row-hover previews before commit, and a hidden-file toggle for people who like hidden files. CRUD happens in place: rename, new file / new folder, move within the workspace, delete (with confirmation for non-recoverable operations). Editing a text file opens it in a new terminal tab running neovim — wired to the workspace's default shell — so the editor you'd reach for on a real shell is the one you reach for here. A read-only git overlay shows the active branch and per-file state (added / modified / untracked / deleted) without ever mutating the working tree.

Selecting a file shows a preview that handles whatever the browser natively renders: text as syntax-highlighted code, images inline, PDFs and SVGs, audio and video, a tabular view of CSVs, a hex fallback for the rest.
